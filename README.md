# Отчет по проекту для курса Обработки речи

## Введение
На работе стоит задача разработки видеокурса, в рамках которой нужно озвучить текст и записать скринкасты. Поэтому цель проекта - попробовать реализовать модель TTS, которая смогла бы озвучивать текст вместо живого человека и сэкономить ресурсы компании.

## Используемые инструменты и архитектура

### Данные и аудиоподготовка
Корпус: [Ruslan](https://ruslan-corpus.github.io/). Не включен в GitHub из-за большого веса(>7Gb). Это открытый датасет на русском языке для задачи преобразования текста в речь. Корпус, состоящий из 22200 высококачественных аудиозаписей речи с соответствующими текстовыми аннотациями общей продолжительностью более 31 часа, является наибольшим по продолжительности датасетом для **одного диктора** на русском языке. По оценкам авторов, сгенерированная с помощью глубокой нейронной сети с этим датасетом речь получила 4.05 балла за естественность и 3.78 за разборчивость по 5-балльной шкале MOS.
...
Базовая модель: [**GlowTTS**](https://jaketae.github.io/study/glowtts/#fnref:2) (flow-based text-to-mel), параметрическая TTS-модель. Преобразует латентное распределение в распределение мел-спектрограмм через обратимое отображение. Взяла как простой и быстрый вариант, так как нет мощной видеокарты(NVIDIA 4060 обычная).
...
- файл `first_output.wav` - модель обучена на 10 эпохах;
- `second_output.wav` - на 20 эпохах;
- `third_output.wav` - на 32 эпохах;
- `fourth_output.wav` - на 50 эпохах;
...
Далее были изменены меры по отношению к градиентам: откачены изменения с предыдущего шага, оставлено только уменьшение Gradient Clipping, установленное в 2.0. 

После чего модель была обучена заново на 90 эпохах. Обучение заняло около 14 часов.
...
`sixth_output.wav` - результат обучения на 100 эпохах. Звук стал менее роботизированным, но звуки не соответствуют тексту.
`last_output.wav` - результат обучения на 130 эпохах. Звук стал еще более похож на человеческую речь, но произношение по-прежнему не соответствует тексту. Обучение заняло более 20 часов.
...
<img width="1426" height="389" alt="image" src="https://github.com/user-attachments/assets/da0a8acf-efc0-4f6e-9ef1-7a8b908a325f" />
<img width="1968" height="930" alt="image" src="https://github.com/user-attachments/assets/ae8896d0-dcc0-4132-a99e-c3e18097ad61" />
...
1. Даже при отсутствии строгих мер на нормы градиентов после 100тыс. шагов обучения показатели перестали повышаться и перешли к плавному понижению(пик - ок. 570). Исходя из этого становится понятно, что принятые изначально меры были неоправданно строгими;
2. `avg_loss` по-прежнему продолжает плавно снижаться даже после 200тыс. шагов, что означает, что модели все еще необходимо дообучаться. Необходимый результат - когда потери выйдут на плато;
3. Следовательно, необходимо попробовать дообучить модель на бОльшем количестве эпох, например 150-200;
4. Можно будет попробовать использовать фонемы при обучении модели;
5. Также в дальнейшем можно будет также поэкспериментировать с другими синтезаторами, может быть, дадут более хороший результат(например, HiFi-GUN).
