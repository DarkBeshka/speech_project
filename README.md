# Отчет по проекту к курсу по обработке речи

## Введение
На работе имею задачу по разработке видеокурса - соответственно, нужно разработать скринкасты и озвучить текст. Цель проекта - попробовать реализовать TTS-модель, которая бы озвучивала нужный мне текст вместо меня.
...
Обработка аудио: `AudioProcessor` генерирует 80-мерные мел-спектрограммы (`fft_size=1024`, `hop_length=256`, `win_length=1024`, `spec_gain=20`, нормализация `signal_norm=True`) - взяла за основу стандартную конфигурацию для вокодера Griffin–Lim.

### Текстовое представление и словарь

Символьный (grapheme) токенизатор с кастомным `CharactersConfig`, в котором расположен полный набор строчных и прописных кириллических символов, а также пунктуацию - они подобраны эмпирически под датасет.

Phonemizer отключен (`use_phonemes=False`) - это упрощает пайплайн, но делает модель чувствительной к орфографии и пунктуации.

### Архитектура модели GlowTTS

Базовая модель: **GlowTTS** (flow-based text-to-mel), параметрическая TTS-модель. Потоки преобразуют распределение $( z \sim \mathcal{N}(0, I) $ в распределение мел-спектрограмм \( x \) через обратимое отображение \( x = f^{-1}(z) \). Максимизация правдоподобия записывается как
  \[
